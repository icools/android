...
這個專案未來會同時被 `Obsidian`、`Codex`、`GitHub Copilot`、`Gemini CLI`、`ChatGPT work with`、`Antigravity` 等工具讀取與操作。

因此規範分成三層：

- `CONTENT_CHARTER.md`
  作為最高準則，定義內容結構、frontmatter、分類、圖片、索引、隱私與 AI 協作邏輯。

- `AI_MARKDOWN_WORKFLOW.md`
  作為給各種 AI 工具快速讀取的短版操作規格，重點放在處理順序、停止條件與必要欄位。

- repo 內 skill
  例如 `_agents/skills/github_pages_doc_audit/SKILL.md`，負責把規範轉成可執行流程。
...
這個 repo 的整理流程，預設不是針對整個專案全面掃描，而是只處理我明確指定的目標。
...
若指定的是資料夾，預設應遞迴處理該資料夾與所有子資料夾底下的 `.md` 檔案。

除非我明確提到要「針對整個專案重新查證」或類似意思，否則 agent 不應自行對整個 repo 做全面檢查、分類或搬移。

預設互動模式採兩階段：

1. 先檢查並提出建議
2. 等我確認後，再進行整理、搬移、補資料與索引更新
...
- `source`
  如果是自行整理內容可填 `local`。
  如果內容明確來自官方文件、GitHub repo 或其他主要來源，應填主要來源網址。
  `source` 只保留單一主要來源，其他重要參考網址應放在正文的參考段落中。

- `status`
  預設使用 `active`。
  若未整理完成，也可以視情況使用 `draft`。
  但如果文章原本來自 `draft/`，已經整理並移入正式分類，仍應以正式文章方式處理，並優先透過 `draft` tag 表示待人工確認，而不是只依賴 `status: draft`。
...
未來若透過 AI 工具協助整理指定檔案或指定資料夾時，應先進行隱私掃描。
...
若文章原本來自 `draft/`，在移入正式分類後還應補做：

8. 在 `tags` 中保留 `draft`，直到我人工確認內容正確。
9. 若是從 `draft/` 移出，才允許自動重新分類與搬移；既有正式文章預設只補齊內容與欄位，不重新搬家。

## Draft 整理規則

`draft/` 是中繼整理區，不是永久分類。

當我指定 `draft/` 中的檔案或資料夾交給 AI 整理時，應遵守以下流程：

1. 先檢查是否命中隱私停止條件。
2. 若未命中，再檢查 frontmatter 是否完整。
3. 從文內擷取主要來源網址，補到 `source`。
4. 根據內容與既有資料夾結構產生適合的 `tags`。
5. 將文章移到最深且已存在的合適分類。
6. 若文章附有本地圖片，同步搬移到目標分類的 `image/`，並更新引用路徑。
7. 在文章移出 `draft/` 後，先保留 `draft` tag，供我後續人工確認。

這裡的「最深且已存在的合適分類」意思是：

- 若 repo 已有 `ai/openai/chatgpt/`，且內容明確屬於該主題，優先放到 `ai/openai/chatgpt/`
- 不要為了單篇文章任意新建過深分類
- 若判斷不明確，優先使用較穩定的既有父層分類
...
若文章原本位於 `draft/`，則應在既有分類中優先選擇最深且已存在的合適路徑，而不是隨意新建分類。
...
若需要補強不確定內容，且文章內已經附有網址，可以再向外查找補充資料。
...
- 預設只處理我指定的檔案或資料夾，不自行掃描整個 repo
- 預設先檢查，再整理；若我要直接整理，會另外明示
- 優先依照本文件決定文章放置位置
- 自動補上 frontmatter
- `source` 只填主要來源，其他連結放到正文
- 自動更新對應的根目錄索引頁
- 若屬於 `ai/` 主題，應同步更新 `ai.md`，必要時也更新對應的 AI 子索引
- 如果我有附上 PNG 圖片，先轉成 JPG，再放進正確分類底下的 `image/`
- 若文章來自 `draft/`，移入正式分類後應保留 `draft` tag，直到我人工確認
- 若文章本來就已在正式分類，預設只補欄位與整理，不重新搬移
- 若命中隱私停止條件，應先停下來，不做任何修改
- 不要把所有內容都塞進 `README.md`
- 工具與專案文章盡量附上官方來源
- 若需要補強不確定內容，可優先參考官方來源與可信技術文章
...
若是針對指定範圍進行整理，可以直接這樣說：

```text
請依照 CONTENT_CHARTER.md 和 AI_MARKDOWN_WORKFLOW.md，先檢查這個檔案或資料夾底下的 Markdown，再提出整理建議；確認後才進行分類、補欄位、搬圖與索引更新。
```

如果只想處理某個既有正式文章，也可以明講：

```text
請依照 CONTENT_CHARTER.md 檢查這篇既有文章的 frontmatter、source、tags 與圖片引用，不要重新分類或搬移文章。
```
